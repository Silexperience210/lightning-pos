...
pub fn blend565(dst: u16, src: u16, a: u8) -> u16 {
    ...
    let a = a as u32;
    let ia = 255 - a;
    let (dr, dg, db) = ((dst >> 11) as u32 & 0x1F, (dst >> 5) as u32 & 0x3F, dst as u32 & 0x1F);
    let (sr, sg, sb) = ((src >> 11) as u32 & 0x1F, (src >> 5) as u32 & 0x3F, src as u32 & 0x1F);
...
impl<'d> Display<'d> {
    ...
    pub fn fill_round_rect_aa(&mut self, x: usize, y: usize, w: usize, h: usize, r: f32, color: u16) {
    ...
    pub fn num_center(&mut self, s: &str, y: usize, size: f32, tracking: f32, color: u16, alpha: u8) {
        let w = crate::glyph::width(s, size, tracking);
        self.draw_num(s, WIDTH.saturating_sub(w) / 2, y, size, tracking, color, alpha);
    }
            ...
            for dy in 0..r {
                for dx in 0..r {
                    ...
                    if ddx * ddx + ddy * ddy <= (r as isize) * (r as isize) {
                        self.set_pixel((cx as isize + ddx) as usize, (cy as isize + ddy) as usize, color);
                    }
                }
            }
        }
